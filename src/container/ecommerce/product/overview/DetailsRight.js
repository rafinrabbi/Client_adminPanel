...
const DetailsRight = ({ product }) => {
  ...
  const {discount, id, product_catagory, product_image, product_name, product_price, product_quantity, product_shop } = product;
  ...
  return (
    <div className="product-details-box__right pdbr">
      ...
      <Rate allowHalf defaultValue={product_price} disabled />
      <span className="pdbr__rating">{product_price}</span>
      <span className="pdbr__review-count"> 778 Reviews</span>
      <p>
        <span className="pdbr__brand-text">Brand :</span>
        <span className="pdbr__brand-name">{product_shop}</span>
      </p>
      <Heading className="pdbr__new-price" as="h3">
        <span className="pdbr__currency">$</span>
        <span className="pdbr__price">{product_price-discount}</span>
      </Heading>
      {product_price && (
        <Heading className="pdbr__old-price" as="h6">
          <del>${product_price}</del> <span className="pdbr__offer-price">30% Off</span>
        </Heading>
      )}
      <p className="pdbr__desc">amar sonar bangla ami tomay valobashi</p>
