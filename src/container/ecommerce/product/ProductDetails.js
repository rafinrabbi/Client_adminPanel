...
const ProductDetails = ({ match }) => {
  ...
  const { products, product } = useSelector(mystate => {
    return {
      product: mystate.product.data,
      products: mystate.products.data,
    };
  });
  ...
  const { img, category } = {img:'',category:'nothing'}
  // let prdDetails =[]
  useEffect(()=>{
    const d = products.filter(prd=>prd.id===match.params.id)
     setState(d[0])
  },[])
  console.log(state)
  return (
              ...
              <Row gutter={30}>
                <Col xs={24} lg={10}>
                  <div className="product-details-box__left pdbl">
                    <figure>
                      <img style={{ width: '100%' }} src={state.product_image} alt="" />
                    </figure>
