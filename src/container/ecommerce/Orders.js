...
import { orderFilter, allOrders } from '../../redux/orders/actionCreator';
 ...
 useEffect(()=>{
  ...
  useEffect(() => {
    ...
    if (orders) {
      ...
      for (const property in orders){
        myOrders.push({...orders[property], id:property})
      }
      setState({
        item: myOrders,
        selectedRowKeys,
  ...
  if (state.item.length) {
    state.item.map((value, key) => {
      console.log(value)
      const { status, orderId, customers, amount, } = value;
      const {cartProducts,date, orderStatus,time, totalPrice, user, id } = value;
      return dataSource.push({
        key: key + 1,
        id: <span className="order-id">{id}</span>,
        customer: <span className="customer-name">{user.name}</span>,
        status: (
          <span
            className={`status ${
              orderStatus.delivered === true ? 'Success' : 'Awaiting Shipment'
            }`}
          >
            {orderStatus.delivered === true ? 'Delivered' : 'Awaiting Shipment'}
          </span>
        ),
        amount: <span className="ordered-amount">{totalPrice}</span>,
